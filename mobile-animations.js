...
function initPreloader() {
    ...
    // Scroll Lock for Preloader (Desktop) - Persistent Loop
    // Use an interval to ensure Lenis is caught if it initializes late
    let preloaderLockInterval = setInterval(() => {
        try {
            window.scrollTo(0, 0);
            disableScroll();
        } catch (e) {
    ...
    if (button) {
        ...
        button.addEventListener("click", () => {
            // Stop the locking loop
            if (preloaderLockInterval) clearInterval(preloaderLockInterval);

            enableScroll(); // Allow scrolling

            // Force Lenis to start if it exists
            if (window.lenis) window.lenis.start();
